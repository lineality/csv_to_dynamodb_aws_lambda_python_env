...
# helper function
def main_split_csv_iterator(number_of_split_files_needed):

    try:
        # Ask how many files to split the file into?
        number_of_final_files = int( number_of_split_files_needed )

        number_of_reductions = int( number_of_final_files / 2 )

    except Exception as e:
 
        output = f"""Error: problem with file splitter input
        Error Message = '{str(e)} 
        """
...
def lambda_handler(event, context):
    ...
    for this_file in plain_names_list:
        ...
        # round up to even
        if (not not number_of_split_files_needed%2) and (number_of_split_files_needed != 1):
            number_of_split_files_needed += 1

        # if number_of_split_files_needed > 1, split it
        if number_of_split_files_needed > 1:

            # for terminal:
            print("Running file splitter, file size over threshold...")

            # Run AWS version of Split Function
            main_split_csv_iterator(number_of_split_files_needed)
